...
RSpec.describe "Projects finance API", type: :request do
  describe "PATCH /api/v1/projects/:api_id/finance/approve" do
    subject(:req) { patch "/api/v1/projects/prj_123/finance/approve" }
    ...
    context "with a project" do
      ...
      context "with finance approval status" do
        ...
        it "returns a project with finance approved status" do
          req

          json = JSON.parse(response.body)

          expect(json["project"]["status"]).to eq("finance_approved")
        end
      end
